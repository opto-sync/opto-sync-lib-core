...
The independent authored authorities live in `opto-sync/opto-sync-interfaces` and are admitted by `ORESoftware/typespec-json-schema-validator`. This Rust implementation is an implementation projection only; generated schemas and this module are evidence, never additional authorities.
