...
fn canonical_boolean_values_are_coerced() {
    let config = parse_opto_sync_config(
        r#"
version = 1
mode = "client"
strict = true
...
"#,
    ...
    for (raw, expected) in [("true", true), ("false", false)] {
        ...
        let resolved =
            resolve_opto_sync_config(&config, &ambient, &BTreeMap::new()).expect("resolved");
        assert_eq!(
            resolved.binding("feature").expect("feature").value(),
            &OptoSyncConfigValue::Bool(expected)
        );
    }
}
