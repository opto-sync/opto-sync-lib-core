...
use opto_sync_lib_core::{
    parse_opto_sync_config, resolve_opto_sync_config, OptoSyncConfigError, OptoSyncConfigValue,
    OptoSyncMode,
};
...
fn rejects_role_mismatch_and_unsafe_contract() {
    let role_error =
        parse_opto_sync_config(&HYBRID.replace("mode = \"hybrid\"", "mode = \"client\""))
            .unwrap_err();
    assert!(matches!(
        role_error,
        OptoSyncConfigError::ModeRoleMismatch(_)
    ));
